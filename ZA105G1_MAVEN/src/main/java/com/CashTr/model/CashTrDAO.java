...
public class CashTrDAO implements CashTrDAO_interface {
...
//								long再放進new java.sql.Date，這邊如果直接用rs.getDate("talk_time")，拿出來的會是sql.Date沒辦法顯示時分秒
	...
	public List<CashTrVO> findusertrandaction(Integer mem_no){
		...
		try {
			con = ds.getConnection();
			String GET_ALL_STMT2 = "SELECT TRANDACTION_DATE,TRANDACTION_NO,MEM_NO,TRANDACTION_MONEY FROM TRANDACTION where mem_no =" + mem_no;
...
//								long再放進new java.sql.Date，這邊如果直接用rs.getDate("talk_time")，拿出來的會是sql.Date沒辦法顯示時分秒
			...
			System.out.println(GET_ALL_STMT2);
...
//-----------------------------------------------------------------------------------------------------------------------//
	...
	public List<CashTrVO> getDay(String date1,String date2){
		...
		try {
			...
			+ date1 + "' and to_char(trandaction_date,'yyyy-mm-dd HH:MI:SS') < '" + date2 +"'" ;
