...
public class CompositeQuery {
	...
	private static String get_aCondition_For_Oracle(String columnName, String value) {		
		...
		//用於Number型態
		if ( "history_no".equals(columnName) || "admin_no".equals(columnName) || "mem_no".equals(columnName) || "store_no".equals(columnName) || "history_state".equals(columnName)  ) {
			aCoundition = columnName + "=" + value;
		}
		else if ( "history_date".equals(columnName) ) {
			aCoundition = columnName + " like '%" + value + "%'";
		}
		else if ( "Update_time".equals(columnName) || "login_in_time".equals(columnName) || "login_out_time".equals(columnName) ) {
...
//		map.put("tickets_type_name",new String[]{ "中文"});
		...
		String finalSQL = "select * from XXX " + get_Where(map) + "order by empno ";
